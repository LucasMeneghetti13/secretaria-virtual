...
function fetchNews(temas = null) {
  return new Promise((resolve) => {
    // Temas em ordem de prioridade
    const TEMAS = temas || [
      { label: "💰 Economia & Finanças", query: "economia financas mercado juros inflacao brasil" },
      { label: "💻 Tecnologia", query: "tecnologia inteligencia artificial inovacao tech" },
      { label: "🏎️ Automobilismo", query: "formula 1 MotoGP automobilismo corrida GP" },
      { label: "📊 Política Econômica", query: "politica economica reforma fiscal governo economia brasil" },
      { label: "🗳️ Política", query: "politica brasil congresso governo eleicoes" }
    ];

    const NEWSAPI_KEY = "7a81c82b82d84c51a99ce66080c6c360";
    const resultados = [];
    let pending = TEMAS.length;

    TEMAS.forEach((tema, idx) => {
      const query = encodeURIComponent(tema.query);
      const path = `/v2/everything?q=${query}&language=pt&sortBy=publishedAt&pageSize=2&apiKey=${NEWSAPI_KEY}`;
      const options = {
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("end", () => {
          try {
            ...
            const articles = (parsed.articles || []).filter(a =>
              a.title && !a.title.includes("[Removed]") && a.source
            );
            if (articles.length > 0) {
              const art = articles[0];
              const fonte = art.source?.name || "Fonte desconhecida";
              const titulo = art.title.replace(/ - .*$/, ""); // remove " - NomeFonte"
              resultados[idx] = `${tema.label}\n<b>${titulo}</b>\n<i>${fonte}</i>`;
            } else {
              resultados[idx] = null;
            }
          } catch(e) {
          ...
          if (pending === 0) {
            ...
            if (validos.length === 0) {
              resolve("Não encontrei notícias recentes nos seus temas agora. Tente novamente em instantes.");
            } else {
              resolve(validos.join("\n\n"));
            }
          }
        });
      });
      req.on("error", () => {
        ...
        if (pending === 0) {
          const validos = resultados.filter(Boolean);
          resolve(validos.length > 0 ? validos.join("\n\n") : "Serviço de notícias indisponível.");
        }
      });
      req.end();
    });
