  ...
  try {
    ...
    for (const uid of novos) {
      ...
      if (classificacao === "urgente") {
        // Guarda o e-mail para possível resposta
        _ultimoEmailUrgente = { from, subject, uid };
        const nomeRemetente = from.split("<")[0].trim().replace(/"/g,'');
        console.log(`🚨 Enviando alerta urgente para Telegram: ${from}`);
        await sendTelegram(`📧 <b>E-mail urgente!</b>\n\n<b>De:</b> ${from}\n<b>Assunto:</b> ${subject}\n\n<i>Para responder, diga: "responde o e-mail do ${nomeRemetente} dizendo..."</i>`);
