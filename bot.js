...
function resumoState() {
  ...
  return `HOJE: ${hoje} | AMANHÃ: ${amanha}
...
TAREFAS PENDENTES (${pendentes.length}): ${pendentes.length ? pendentes.slice(0,5).map(t=>`${t.nome} [${t.prio}]${t.prazo?" prazo:"+t.prazo:""}`).join("; ") : "nenhuma"}
...
async function executeAction(jsonStr) {
  ...
  try {
    ...
    if (!match) {
      // Se não tem JSON, trata como resposta direta
      return clean || "Não entendi. Pode repetir de outra forma?";
    }
    action = JSON.parse(match[0]);
  } catch(e) {
    // Se falhou o parse, retorna o texto limpo como resposta
    const clean = jsonStr.replace(/```json\s*/gi,'').replace(/```\s*/gi,'').trim();
