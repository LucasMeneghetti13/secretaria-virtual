...
function resumoState() {
  ...
  const horaBRT = agora.toLocaleTimeString("pt-BR", {timeZone:"America/Sao_Paulo", hour:"2-digit", minute:"2-digit"});
  const agoraBRT = hoje + "T" + horaBRT.replace(":",":") + ":00-03:00";
  ...
  const eventosHoje = state.eventos.filter(e=>e.data===hoje && !e.concluido);
  const eventosAmanha = state.eventos.filter(e=>e.data===amanhaStr && !e.concluido);
  const proxEventos = state.eventos.filter(e=>e.data>hoje && !e.concluido).sort((a,b)=>a.data>b.data?1:-1).slice(0,5);
  const eventosConcluidosHoje = state.eventos.filter(e=>e.data===hoje && e.concluido);
  return `AGORA: ${hoje} ${horaBRT} (${diaSemana}) | AMANHÃ: ${amanhaStr}
TIMESTAMP_ATUAL_BRT: ${agoraBRT}
PRÓXIMOS DIAS: ${diasRef.join(" | ")}
FINANÇAS: receitas ${fmt(rec)} | despesas ${fmt(desp)} | saldo ${fmt(rec-desp)}
EVENTOS HOJE EM ABERTO (${eventosHoje.length}): ${eventosHoje.length ? eventosHoje.map(e=>`${e.titulo}${e.hora?" "+e.hora:""}`).join("; ") : "nenhum"}
EVENTOS HOJE JÁ CONCLUÍDOS (${eventosConcluidosHoje.length}): ${eventosConcluidosHoje.length ? eventosConcluidosHoje.map(e=>e.titulo).join("; ") : "nenhum"}
EVENTOS AMANHÃ (${eventosAmanha.length}): ${eventosAmanha.length ? eventosAmanha.map(e=>`${e.titulo}${e.hora?" "+e.hora:""}`).join("; ") : "nenhum"}
PRÓXIMOS EVENTOS EM ABERTO: ${proxEventos.length ? proxEventos.map(e=>`${e.titulo} em ${e.data}${e.hora?" "+e.hora:""}`).join("; ") : "nenhum"}
TAREFAS PENDENTES (${pendentes.length}): ${pendentes.length ? pendentes.map(t=>`${t.nome} [${t.prio}]${t.prazo?" prazo:"+t.prazo:""}`).join("; ") : "nenhuma"}
METAS: ${state.metas.length ? state.metas.map(m=>`${m.nome} ${m.alvo>0?Math.round(m.atual/m.alvo*100):0}%`).join(", ") : "nenhuma"}
LEMBRETES ATIVOS: ${state.lembretes.length}
NOTAS SALVAS: ${state.notas.length}${state.notas.length ? ' (temas: '+state.notas.slice(0,5).map(n=>n.titulo).join(', ')+')' : ''}`.trim();
}
...
Para EDITAR TAREFA:
{"acao":"editar_tarefa","nome_atual":"...","nome":"novo nome","prio":"alta|media|baixa","prazo":"YYYY-MM-DD","cat":"..."}

Para DELETAR TAREFA:
{"acao":"deletar_tarefa","nome":"..."}
...
Para RESPONDER E-MAIL urgente recebido (quando pedir para responder um e-mail):
{"acao":"rascunho_email","orientacao":"o que o Lucas quer dizer de forma simples"}

Para AJUSTAR RASCUNHO (quando reprovar e pedir mudança no rascunho):
{"acao":"ajustar_rascunho","orientacao":"instrução de mudança do Lucas"}

Para ENVIAR E-MAIL APROVADO (quando Lucas disser: envia, pode enviar, manda o email):
{"acao":"enviar_rascunho"}

Para CRIAR LEMBRETE (quando pedir para lembrar, avisar, notificar em uma data/hora):
{"acao":"criar_lembrete","texto":"...","data_hora":"YYYY-MM-DDTHH:MM:00-03:00"}
Regras OBRIGATÓRIAS para data_hora — use SEMPRE o TIMESTAMP_ATUAL_BRT como base:
- "daqui N minutos" = TIMESTAMP_ATUAL_BRT + N minutos
- "daqui N horas" = TIMESTAMP_ATUAL_BRT + N horas
- "amanhã" = AMANHÃ às 09:00-03:00
- "sexta" ou outro dia = data do PRÓXIMOS DIAS correspondente às 09:00-03:00
- "às 14h" = use o horário informado no dia correto
- Use SEMPRE fuso -03:00 (Brasília)
- NUNCA use horário anterior ao TIMESTAMP_ATUAL_BRT

Para CRIAR MÚLTIPLOS LEMBRETES:
{"acao":"criar_lembretes","lembretes":[{"texto":"...","data_hora":"YYYY-MM-DDTHH:MM:00-03:00"},{"texto":"...","data_hora":"YYYY-MM-DDTHH:MM:00-03:00"}]}
...
Para MARCAR MÚLTIPLOS EVENTOS COMO CONCLUÍDOS:
{"acao":"concluir_eventos","titulos":["evento 1","evento 2","..."]}

Para CRIAR NOTA (quando pedir para anotar, registrar, guardar uma informação):
{"acao":"criar_nota","titulo":"...","corpo":"conteudo completo da nota","pasta":"Geral|Trabalho|Pessoal"}

Para BUSCAR NOTA (quando pedir para ver, recuperar ou consultar uma anotação):
{"acao":"buscar_nota","busca":"palavra chave ou tema da nota"}
...
Para CONSULTAR ÚLTIMA TRANSAÇÃO (quando perguntar qual foi a última despesa/receita, onde parou, última movimentação, última da conta X):
{"acao":"ultima_transacao","conta":"nome da conta ou vazio","tipo":"despesa|receita|vazio"}

{"acao":"relatorio","tipo":"despesa|receita|todos","mes":"01-12 ou vazio","ano":"2026 ou vazio","cat":"categoria ou vazio","conta":"conta ou vazio","data_ini":"YYYY-MM-DD ou vazio","data_fim":"YYYY-MM-DD ou vazio"}
Quando informar período (ex: "do dia 20/06 até 06/07"), use data_ini e data_fim em vez de mes/ano.
...
async function executeAction(jsonStr) {
  ...
  switch(action.acao) {
    ...
    case "concluir_eventos": {
      const titulos = action.titulos || [];
      const concluidos = [];
      const naoEncontrados = [];
      for (const titulo of titulos) {
        const ev = state.eventos.find(e =>
          !e.concluido && e.titulo.toLowerCase().includes(titulo.toLowerCase())
        );
        if (ev) {
          ev.concluido = true;
          await sbUpdate("eventos", ev.id, { concluido: true });
          concluidos.push(ev.titulo);
        } else {
          naoEncontrados.push(titulo);
        }
      }
      let msg = `✅ <b>${concluidos.length} compromisso(s) marcado(s) como feito!</b>\n`;
      concluidos.forEach(t => msg += `• ${t}\n`);
      if (naoEncontrados.length) msg += `\n⚠️ Não encontrei: ${naoEncontrados.join(", ")}`;
      return msg;
    }

    case "editar_tarefa": {
      const t = state.tarefas.find(t =>
        t.nome.toLowerCase().includes((action.nome_atual||action.nome).toLowerCase())
      );
      if (t) {
        if (action.nome && action.nome_atual) t.nome = action.nome;
        if (action.prio) t.prio = action.prio;
        if (action.prazo) t.prazo = action.prazo;
        if (action.cat) t.cat = action.cat;
        await sbUpdate("tarefas", t.id, {nome:t.nome, prio:t.prio, prazo:t.prazo||"", cat:t.cat, done:t.done});
        return `✏️ Tarefa "<b>${t.nome}</b>" atualizada!`;
      }
    ...
    case "deletar_tarefa": {
      const t = state.tarefas.find(t =>
        t.nome.toLowerCase().includes(action.nome.toLowerCase())
      );
      if (t) {
        await sbDelete("tarefas", t.id);
        state.tarefas = state.tarefas.filter(x => x.id !== t.id);
        return `🗑️ Tarefa "<b>${t.nome}</b>" removida.`;
      }
      return `Não encontrei essa tarefa.`;
    }
    ...
    case "criar_nota": {
      const nota = {
        id: Date.now(),
        titulo: action.titulo || "Nota",
        corpo: action.corpo || action.conteudo || "",
        pasta: action.pasta || "Geral",
        tag: action.tag || "Geral",
        data: hojeStr()
      };
      await sbInsert("notas", nota);
      state.notas.push(nota);
      return `📝 <b>Nota salva!</b>\n\n<b>${nota.titulo}</b>\n${nota.corpo}\n📂 ${nota.pasta}`;
    }

    case "buscar_nota": {
      const busca = (action.busca || "").toLowerCase();
      const encontradas = state.notas.filter(n =>
        (n.titulo||"").toLowerCase().includes(busca) ||
        (n.corpo||"").toLowerCase().includes(busca) ||
        (n.pasta||"").toLowerCase().includes(busca)
      );
      if (!encontradas.length) {
        // Tenta recarregar do Supabase antes de desistir
        const nt = await sbGet("notas");
        state.notas = nt;
        const encontradas2 = state.notas.filter(n =>
          (n.titulo||"").toLowerCase().includes(busca) ||
          (n.corpo||"").toLowerCase().includes(busca) ||
          (n.pasta||"").toLowerCase().includes(busca)
        );
        if (!encontradas2.length) return `📭 Não encontrei nenhuma nota sobre "${action.busca}". Quer que eu crie uma?`;
        return formatarNotas(encontradas2);
      }
      return formatarNotas(encontradas);
    }
    ...
    case "criar_lembrete": {
      ...
      const opts = { timeZone:"America/Sao_Paulo", day:"2-digit", month:"2-digit", year:"numeric", hour:"2-digit", minute:"2-digit" };
      const dtFmt = dataHora.toLocaleString("pt-BR", opts);
      return `⏰ <b>Lembrete criado!</b>\n\n📝 ${action.texto}\n🕐 ${dtFmt}`;
    }

    case "criar_lembretes": {
      const criados = [];
      for (let i = 0; i < action.lembretes.length; i++) {
        ...
        if (i < action.lembretes.length - 1) await new Promise(r => setTimeout(r, 50));
      }
      return `⏰ <b>${criados.length} lembrete(s) criado(s)!</b>\n\n${criados.join("\n")}`;
    }
    ...
    case "rascunho_email": {
      if (!_ultimoEmailUrgente) {
        return `📧 Não há e-mail urgente recente para responder. Aguarde chegar um novo e-mail.`;
      }
      const { from, subject } = _ultimoEmailUrgente;
      const nomeRem = from.split("<")[0].trim().replace(/"/g,'');
      const para = from.match(/<(.+)>/) ? from.match(/<(.+)>/)[1] : from;
      const assunto = subject.startsWith("Re:") ? subject : `Re: ${subject}`;

      // Gera rascunho profissional com IA
      const rascunhoBody = JSON.stringify({
        model: "claude-sonnet-4-5",
        max_tokens: 500,
        messages: [{
          role: "user",
          content: `Escreva um e-mail profissional de resposta em português brasileiro. 
Remetente original: ${from}
Assunto: ${subject}
O que o Lucas quer dizer: "${action.orientacao}"

Escreva APENAS o corpo do e-mail, sem saudação de abertura com nome (já será adicionada), sem "Atenciosamente" no final (será adicionado automaticamente). Seja profissional mas natural. Máximo 150 palavras.`
        }]
      });

      const corpoPuro = await callAnthropic(rascunhoBody);
      const corpoFinal = `Olá ${nomeRem},\n\n${corpoPuro.trim()}\n\nAtenciosamente,\nLucas Meneghetti`;

      _rascunhoEmail = { para, assunto, corpo: corpoFinal, tentativas: 1 };

      return `✏️ <b>Rascunho preparado:</b>\n\n<b>Para:</b> ${nomeRem}\n<b>Assunto:</b> ${assunto}\n\n${corpoFinal}\n\n<i>Diga <b>"envia"</b> para enviar ou me fale o que mudar no texto.</i>`;
    }

    case "ajustar_rascunho": {
      if (!_rascunhoEmail) {
        return `Não há rascunho pendente. Peça para responder um e-mail primeiro.`;
      }
      if (_rascunhoEmail.tentativas >= 3) {
        const rascunho = _rascunhoEmail.corpo;
        _rascunhoEmail = null;
        return `😅 Não consegui acertar em 3 tentativas. Aqui está o rascunho final para você editar e enviar manualmente:\n\n${rascunho}`;
      }

      // Refaz o rascunho com a orientação de mudança
      const { para, assunto } = _rascunhoEmail;
      const nomeRem = para.split("@")[0];

      const ajusteBody = JSON.stringify({
        model: "claude-sonnet-4-5",
        max_tokens: 500,
        messages: [{
          role: "user",
          content: `Reescreva este e-mail aplicando a seguinte mudança: "${action.orientacao}"\n\nTexto atual:\n${_rascunhoEmail.corpo}\n\nRetorne APENAS o novo texto do e-mail completo, sem comentários.`
        }]
      });

      const novoCorpo = await callAnthropic(ajusteBody);
      _rascunhoEmail.corpo = novoCorpo.trim();
      _rascunhoEmail.tentativas++;

      return `✏️ <b>Rascunho revisado (tentativa ${_rascunhoEmail.tentativas}/3):</b>\n\n${_rascunhoEmail.corpo}\n\n<i>Diga <b>"envia"</b> para enviar ou me fale o que mudar.</i>`;
    }

    case "enviar_rascunho": {
      if (!_rascunhoEmail) {
        return `Não há rascunho pendente para enviar.`;
      }
      const { para, assunto, corpo } = _rascunhoEmail;
      try {
        await enviarEmail(para, assunto, corpo);
        const nomeRem = para.split("@")[0];
        _rascunhoEmail = null;
        return `✅ <b>E-mail enviado com sucesso!</b>\n\n<b>Para:</b> ${para}\n<b>Assunto:</b> ${assunto}\n\nO destinatário receberá sua resposta em instantes. 📬`;
      } catch(e) {
        return `❌ Erro ao enviar: ${e.message}\n\nVerifique sua conexão e tente novamente com "envia".`;
      }
    }
    ...
    case "ultima_transacao": {
      let lista = state.transacoes.slice();
      if (action.conta) {
        const contaBusca = action.conta.toLowerCase().trim();
        // Tenta correspondência exata primeiro, depois parcial
        const exata = lista.filter(t => t.conta && t.conta.toLowerCase() === contaBusca);
        if (exata.length > 0) {
          lista = exata;
        } else {
          // Correspondência parcial mas evita substrings — ex: "sicoob" não pega "sicoob cartão"
          lista = lista.filter(t => {
            if (!t.conta) return false;
            const contaT = t.conta.toLowerCase();
            // Se a busca é mais curta, verifica se é uma palavra completa da conta
            if (contaT === contaBusca) return true;
            if (contaT.startsWith(contaBusca + ' ') || contaT.endsWith(' ' + contaBusca)) return true;
            if (contaT.includes(' ' + contaBusca + ' ')) return true;
            return false;
          });
          // Se não achou com palavra exata, usa parcial mesmo
          if (!lista.length) lista = state.transacoes.filter(t => t.conta && t.conta.toLowerCase().includes(contaBusca));
        }
      }
      if (action.tipo && action.tipo !== "vazio") lista = lista.filter(t => t.tipo === action.tipo);
      if (!lista.length) return `Nenhuma transação encontrada${action.conta ? " na conta " + action.conta : ""}.`;

      lista.sort((a,b) => (b.data > a.data ? 1 : b.data < a.data ? -1 : 0));
      const ult = lista[0];
      const d = (ult.data||"").split("-");
      const dataFmt = d.length === 3 ? `${d[2]}/${d[1]}/${d[0]}` : ult.data;
      const emoji = ult.tipo === "receita" ? "💰" : "💸";
      const sinal = ult.tipo === "receita" ? "+" : "-";

      let msg = `${emoji} <b>Última ${ult.tipo} na ${ult.conta}:</b>\n\n`;
      msg += `📝 ${ult.desc || ult.descricao}\n`;
      msg += `💳 ${ult.conta || "sem conta"}\n`;
      msg += `📅 ${dataFmt}\n`;
      msg += `💵 ${sinal}${fmt(ult.valor)}\n\n`;

      const anteriores = lista.slice(1, 4);
      if (anteriores.length) {
        msg += `<i>Anteriores:\n`;
        anteriores.forEach(t => {
          const d2 = (t.data||"").split("-");
          const dt = d2.length === 3 ? `${d2[2]}/${d2[1]}` : t.data;
          msg += `• ${dt} ${t.desc||t.descricao} — ${fmt(t.valor)}\n`;
        });
        msg += `</i>`;
      }
      return msg;
    ...
    case "relatorio": {
      const filtradas = state.transacoes.filter(t => {
        ...
        // Filtro por período
        if (action.data_ini && dtBase < action.data_ini) return false;
        if (action.data_fim && dtBase > action.data_fim) return false;
        // Filtro por mês/ano (se não usar período)
        if (!action.data_ini && !action.data_fim) {
      ...
      // Monta período/título
      let periodo = '';
      if (action.data_ini || action.data_fim) {
        const fmtD = d => d ? d.split('-').reverse().join('/') : '';
        periodo = `${fmtD(action.data_ini)} a ${fmtD(action.data_fim)}`;
      } else {
        periodo = action.mes ? mesesNome[parseInt(action.mes)-1]+(action.ano?'/'+action.ano:'') : (action.ano||'todos os períodos');
      }
      ...
      if (filtradas.length > 15) msg += `<i>... e mais ${filtradas.length-15} lançamentos. Ver completo no painel.</i>\n`;
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          if (!resp.ok) {
            console.error(`❌ Telegram erro: ${resp.description}`);
            // Tenta sem parse_mode se falhar por HTML
            const body2 = JSON.stringify({ chat_id: chatId, text: text.replace(/<[^>]+>/g,'') });
            const req2 = https.request({...options, headers: {...options.headers, "Content-Length": Buffer.byteLength(body2)}}, (res2) => {
              let d2=""; res2.on("data",c=>d2+=c); res2.on("end",()=>resolve(JSON.parse(d2)));
            });
            req2.on("error", e => resolve(null));
...
let _ultimoEmailUrgente = null; // último e-mail urgente para resposta
let _emailsUrgentesHistorico = []; // histórico de urgentes do dia
let _rascunhoEmail = null; // rascunho aguardando aprovação
// _rascunhoEmail = { para, assunto, corpo, tentativas }
...
function enviarEmail(para, assunto, corpo) {
  return new Promise((resolve, reject) => {
    ...
    const socket = tls.connect({
      host: EMAIL_CONFIG.host, port: 465, rejectUnauthorized: false
    });
    let buffer = "";
    let step = 0;
    ...
    socket.on("data", d => {
      ...
      if (step === 0 && line.startsWith("220")) { step=1; next(`EHLO mail.meneghettimoveis.com.br`); }
      else if (step === 1 && (line.startsWith("250") || line.includes("250 "))) { step=2; next(`AUTH LOGIN`); }
      else if (step === 2 && line.startsWith("334")) { step=3; next(Buffer.from(EMAIL_CONFIG.user).toString("base64")); }
      else if (step === 3 && line.startsWith("334")) { step=4; next(Buffer.from(EMAIL_CONFIG.pass).toString("base64")); }
      else if (step === 4 && line.startsWith("235")) { step=5; next(`MAIL FROM:<${EMAIL_CONFIG.user}>`); }
      else if (step === 5 && line.startsWith("250")) { step=6; next(`RCPT TO:<${para}>`); }
      else if (step === 6 && line.startsWith("250")) { step=7; next(`DATA`); }
      else if (step === 7 && line.startsWith("354")) {
        step=8;
        const msg = [
          `From: Lucas Meneghetti <${EMAIL_CONFIG.user}>`,
          `To: ${para}`,
          `Subject: ${assunto}`,
          `MIME-Version: 1.0`,
      ...
      else if (step === 8 && line.startsWith("250")) { step=9; next(`QUIT`); resolve(); socket.destroy(); }
      else if (line.startsWith("5")) { reject(new Error(line)); socket.destroy(); }
    });

    socket.on("error", reject);
    socket.setTimeout(30000, () => { reject(new Error("Timeout SMTP")); socket.destroy(); });
  });
}

async function buscarEmailsNovos() {
  try {
    // Recarrega config para garantir lista branca atualizada
    const cfgRes = await sbRequest("GET", "configuracoes", null, "?chave=eq.cfg&select=valor");
    if (cfgRes.data?.[0]?.valor) {
      try {
        const userCfg = JSON.parse(cfgRes.data[0].valor);
        if (userCfg.emailsUrgentes) state.emailsUrgentes = userCfg.emailsUrgentes;
      } catch(e) {}
    }
    console.log(`📋 Lista branca e-mails (${state.emailsUrgentes.length}): ${state.emailsUrgentes.join(', ') || 'vazia'}`);
    ...
    for (const uid of novos) {
      ...
      const fetchRes = await sendCmd(`FETCH ${uid} (BODY[HEADER.FIELDS (FROM SUBJECT DATE)] BODY[TEXT]<0.1000>)`);
      ...
      // Extrai corpo do e-mail (texto simples)
      const bodyMatch = fetchRes.match(/\r\n\r\n([\s\S]+?)(?:\r\n\.\r\n|$)/);
      let bodyText = "";
      if (bodyMatch) {
        bodyText = bodyMatch[1]
          .replace(/=\r\n/g,'') // quoted-printable line breaks
          .replace(/=[0-9A-F]{2}/gi, '') // encoded chars
          .replace(/<[^>]+>/g, ' ') // HTML tags
          .replace(/\s+/g, ' ')
          .trim()
          .substring(0, 300);
      ...
      // Classifica com IA
      console.log(`📧 E-mail de: ${from} | Assunto: ${subject}`);
      console.log(`📋 Lista branca (${state.emailsUrgentes.length}): ${state.emailsUrgentes.join(', ')}`);
      ...
      if (classificacao === "urgente") {
        _ultimoEmailUrgente = { from, subject, uid, body: bodyText };
        const nomeRemetente = from.replace(/<[^>]+>/g,'').replace(/"/g,'').trim() || from;
        const emailRemetente = from.match(/<(.+)>/)?.[1] || from;
        console.log(`🚨 Enviando alerta urgente para Telegram: ${emailRemetente}`);
        
        let msg = `📧 <b>E-mail urgente!</b>\n\n`;
        msg += `<b>De:</b> ${nomeRemetente}\n`;
        msg += `<b>Assunto:</b> ${subject}\n\n`;
        if (bodyText) {
          msg += `<b>Conteúdo:</b>\n${bodyText}`;
          if (bodyText.length >= 300) msg += `\n<i>... (mensagem truncada)</i>`;
        } else {
          msg += `<i>(sem prévia disponível — pode conter anexo)</i>`;
        }
        msg += `\n\nPara responder diga: "responde o e-mail dizendo..."`;
        
        await sendTelegram(msg);
        console.log(`✅ Alerta enviado!`);
      }
      // Informativos e spam: ignora silenciosamente
    }
...
async function classificarEmail(from, subject) {
  ...
  // REGRA PRINCIPAL: só urgente se estiver na lista branca
  const whitelist = state.emailsUrgentes || [];
  if (whitelist.some(w => fromLower.includes(w.toLowerCase()))) return "urgente";

  // Spam óbvio — ignora silenciosamente
  const spamPatterns = ["unsubscribe","descadastrar","newsletter","promoção","oferta",
    ...
    "fatura eletrônica","não responda","automaticamente gerado","postman.","promaster.",
    "mkt@","news@","info@","contato@post","newsletter@","mailer@"];
  if (spamPatterns.some(p => fromLower.includes(p) || subLower.includes(p))) return "spam";

  // Todo o resto é informativo — não urgente, não spam
  return "informativo";
}
...
function formatarNotas(notas) {
  if (notas.length === 1) {
    const n = notas[0];
    return `📝 <b>${n.titulo}</b>\n📂 ${n.pasta||'Geral'} · ${n.data||''}\n\n${n.corpo||''}`;
  }
  let msg = `📝 <b>${notas.length} anotações encontradas:</b>\n\n`;
  notas.slice(0,5).forEach((n,i) => {
    const preview = (n.corpo||'').substring(0,100);
    msg += `<b>${i+1}. ${n.titulo}</b> (${n.pasta||'Geral'})\n${preview}${preview.length>=100?'...':''}\n\n`;
  });
  return msg;
}
...
async function processMessage(text) {
  ...
  try {
    // Detecta aprovação de rascunho de e-mail
    const aprovacaoPatterns = /^(envia|pode enviar|manda o email|manda o e-mail|envia o email|envia o e-mail|ok envia|pode mandar|manda)[\s!.]*$/i;
    if (_rascunhoEmail && aprovacaoPatterns.test(text.trim())) {
      const resposta = await executeAction(JSON.stringify({acao:"enviar_rascunho"}));
      await sendTelegram(resposta);
      return;
    }

    // Detecta reprovação/ajuste quando há rascunho pendente
    const reprovaPatterns = /^(não|nao|muda|altera|ajusta|refaz|corrige|deixa mais|coloca|tira|adiciona)/i;
    if (_rascunhoEmail && reprovaPatterns.test(text.trim())) {
      const resposta = await executeAction(JSON.stringify({acao:"ajustar_rascunho", orientacao: text}));
      await sendTelegram(resposta);
      return;
    }
    ...
    // Limpa histórico para perguntas de consulta de status — evita respostas contaminadas
    const consultaPatterns = /compromisso|tarefa|pendente|aberto|agenda|evento|concluíd|realizei|atrasad/i;
    if (textProcessado !== text || consultaPatterns.test(text)) {
      state.conversationHistory = [];
      console.log(`🗑️ Histórico limpo — ${textProcessado !== text ? 'data resolvida' : 'consulta de status'}`);
    }
...
async function briefingManha() {
  ...
  // Informativos não aparecem mais no briefing — só urgentes chegam na hora
  _emailsInformativos = []; // limpa acumulados
...
async function checkBriefings() {
  ...
  const agora = Date.now();
  for (const l of state.lembretes) {
    if (!l.enviado && l.dataHora <= agora) {
      l.enviado = true;
      await sbUpdate("lembretes", l.id, { enviado: true });
      await sendTelegram(`⏰ <b>Lembrete!</b>\n\n${l.texto}`);
    }
  }
  state.lembretes = state.lembretes.filter(l => !l.enviado);
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url === "/calendar.ics") {
    ...
    const toICSDateTime = (data, hora) => {
      const base = toICS(data);
      if (!base) return '';
      if (!hora) return base;
      const [h, m] = hora.split(':');
      // Retorna em horário de Brasília com timezone
      return `TZID=America/Sao_Paulo:${base}T${pad(h)}${pad(m)}00`;
    };
    ...
    let ics = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//Claudete//Secretaria Virtual//PT',
      'CALSCALE:GREGORIAN',
      'METHOD:PUBLISH',
    ...
    eventos.forEach(e => {
      ...
      if (e.diaTodo || !e.hora) {
        ...
        dtend = `TZID=America/Sao_Paulo:${toICS(e.data)}T${pad(endH)}${pad(m)}00`;
      }
      ...
      if (e.tipo) ics.push('CATEGORIES:' + e.tipo);
      ics.push('END:VEVENT');
    });
  ...
  // ── GET /api/notas ──
  if (req.method === "GET" && url === "/api/notas") {
    const notas = await sbGet("notas");
    res.writeHead(200, CORS);
