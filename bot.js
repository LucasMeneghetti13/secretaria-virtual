...
async function loadFromSupabase() {
  try {
    ...
    state.eventos    = ev.map(e => ({...e, desc: e.descricao, diaTodo: e.dia_todo}));
    state.tarefas    = tk;
    state.metas      = mt;
...
Para VERIFICAR E-MAILS (quando perguntar sobre e-mails, caixa de entrada, mensagens novas):
{"acao":"verificar_emails"}
...
async function fetchNews(categoria) {
  ...
  if (_newsCache.texto && agora - _newsCache.time < 60 * 60 * 1000) {
    return _newsCache.texto;
  }
  try {
    const temas = [
      { nome: "💰 Economia", query: "economia brasil" },
      { nome: "💻 Tecnologia", query: "tecnologia inteligencia artificial" },
      { nome: "🏎️ Automobilismo", query: "formula 1 MotoGP" },
      { nome: "📊 Mercado", query: "mercado financeiro bolsa brasil" },
      { nome: "🗳️ Política", query: "politica brasil governo" }
    ];
...
async function classificarEmail(from, subject) {
  // Regras rápidas sem chamar IA
  const spamPatterns = ["unsubscribe","descadastrar","newsletter","promoção","oferta","desconto",
    "sale","noreply","no-reply","marketing","notify@","notification@","donotreply"];
  ...
  // Domínios conhecidos de empresas = informativo
  if (fromLower.includes("@") && !fromLower.includes("gmail") && !fromLower.includes("hotmail")) {
    return "informativo";
  }

  // Chama IA para classificar o resto
  try {
    const body = JSON.stringify({
      model: "claude-sonnet-4-5",
      max_tokens: 20,
      messages: [{ role: "user", content: `Classifique este e-mail. Responda APENAS: urgente, informativo ou spam.\n\nDe: ${from}\nAssunto: ${subject}` }]
    });
    const res = await callAnthropic(body);
    const txt = res.toLowerCase();
    if (txt.includes("urgente")) return "urgente";
    if (txt.includes("informativo")) return "informativo";
    return "spam";
  } catch(e) {
    return "informativo"; // dúvida = informativo
  }
}
  ...
  if (req.method === "PUT" && url.startsWith("/api/evento/")) {
    ...
    req.on("end", async () => {
      try {
        ...
        if (e) { Object.assign(e, data); await sbUpdate("eventos", id, {titulo:e.titulo,data:e.data,hora:e.hora||"",tipo:e.tipo,link:e.link||""}); }
        res.writeHead(200, CORS);
        res.end(JSON.stringify({ ok: true }));
      } catch(e) { res.writeHead(400, CORS); res.end(JSON.stringify({ ok: false })); }
    });
