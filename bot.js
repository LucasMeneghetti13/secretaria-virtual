...
async function processMessage(text) {
  ...
  // Helper: verifica se o texto contém qualquer uma das palavras
  const has = (...words) => words.some(w => t.includes(w));

  // LANÇAMENTO RÁPIDO: "paguei 150 almoço" ou "recebi 3000 salário"
  const padraoDesp = /(?:paguei|gastei|comprei|debitou|saiu|pagar)\s+([\d,\.]+)\s+(.+)/i;
  const padraoRec  = /(?:recebi|entrou|ganhei|depositou|receber)\s+([\d,\.]+)\s+(.+)/i;
  ...
  // SALDO / FINANCAS
  if (has("saldo", "financ", "dinheiro", "quanto tenho", "extrato", "gastos")) {
    let rec = 0, desp = 0;
  ...
  // AGENDA
  if (has("agenda", "hoje", "evento", "compromisso", "reuniao", "horario", "dia")) {
    const dataHoje = hojeStr();
  ...
  // TAREFAS
  if (has("tarefa", "task", "pendente", "fazer", "afazeres", "lista")) {
  ...
  // METAS
  if (has("meta", "objetivo", "progresso", "sonho", "planejamento")) {
    if (state.metas.length === 0) {
  ...
  // NOTICIAS
  if (has("noticia", "news", "novidade", "aconteceu", "mundo", "economia")) {
    const noticias = await fetchNews();
    return await sendMessage(`📰 <b>Notícias agora</b>\n\n${noticias}`);
  }

  // AJUDA
  if (has("ajuda", "help", "comando", "o que voce faz", "o que você faz") || t === "/start" || t === "/help") {
    return await sendMessage(
