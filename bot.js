...
async function loadFromSupabase() {
  try {
    const [tr, ev, tk, mt, hb] = await Promise.all([
    ...
    console.log("📂 Supabase: " + state.transacoes.length + " transações, " + state.eventos.length + " eventos, " + state.tarefas.length + " tarefas");
...
Para GERAR RESUMO DE RELATÓRIO (quando pedir relatório, extrato ou reembolso):
  ...
  try {
    ...
    if (updates.ok && updates.result.length > 0) {
      for (const update of updates.result) {
        ...
        // Ignora mensagens de outros bots e canais (spam como A-TOOLS X)
