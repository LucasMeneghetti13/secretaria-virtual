...
async function loadFromSupabase() {
  try {
    ...
    console.log("📂 Supabase: " + state.transacoes.length + " transações, " + state.eventos.length + " eventos, " + state.tarefas.length + " tarefas, " + state.lembretes.length + " lembretes | cats: " + (state.userCats||[]).join(', '));
...
LEMBRETES ATIVOS: ${state.lembretes.length}`.trim();
}
...
async function executeAction(jsonStr) {
  ...
  switch(action.acao) {
    ...
    case "criar_nota": {
      // Salva como tarefa com categoria Notas por ora (notas precisam de API própria)
      const nota = {
        id: Date.now(),
        titulo: action.titulo || "Nota",
        conteudo: action.conteudo || "",
      ...
      return `📝 <b>Nota registrada!</b>\n\n<b>${nota.titulo}</b>\n${nota.conteudo}\n📂 ${nota.pasta}\n\n<i>Abra o painel para ver suas notas.</i>`;
    }
