...
function loadData() {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      console.log(`📂 Dados carregados: ${saved.transacoes?.length||0} transações, ${saved.eventos?.length||0} eventos, ${saved.tarefas?.length||0} tarefas`);
      return saved;
    }
  } catch(e) {
...
let state = {
  transacoes: _saved.transacoes || [],
  tarefas: _saved.tarefas || [],
  eventos: _saved.eventos || [],
  metas: _saved.metas || [],
