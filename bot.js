...
function enviarEmail(para, assunto, corpo) {
  return new Promise((resolve, reject) => {
    ...
    socket.on("error", reject);
    socket.setTimeout(15000, () => { reject(new Error("Timeout SMTP")); socket.destroy(); });
  });
}
  ...
  try {
    ...
    for (const uid of novos) {
      ...
      if (classificacao === "urgente") {
        _ultimoEmailUrgente = { from, subject, uid };
        // Limpa o nome do remetente para evitar problemas com HTML
...
async function classificarEmail(from, subject) {
  ...
  // Lista branca — verifica e-mail completo apenas
